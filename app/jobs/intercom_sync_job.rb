  ...
  def perform(user)
    ...
    intercom_user = begin
      ...
      if user.metadata&.dig("intercom_user_id").present?
        intercom.contacts.find(id: user.metadata.dig("intercom_user_id"))
      end
    ...
    unless intercom_user.custom_attributes["shop_owner"].present?
      intercom_user.custom_attributes["shop_owner"] = user.metadata.dig("shop_owner")
    end
